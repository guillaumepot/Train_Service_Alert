...
from src.extract import fetch_gtfs_rt, extract_ids_and_trip_updates, is_already_sent

# --- fetch_gtfs_rt ---
def test_fetch_gtfs_rt_returns_none_on_error():
    with patch('src.extract.requests.get', side_effect = Exception('fail')):
...
def test_extract_ids_and_trip_updates_trip_update(monkeypatch):
    ...
    monkeypatch.setattr('src.extract.MessageToDict', lambda x: x)
